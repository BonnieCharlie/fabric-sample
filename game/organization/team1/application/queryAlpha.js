 ...
 * 4. Construct request to quey the ledger and obtain alpha value
...
async function main() {
    ...
    try {
        ...
        let alphaResponse = await contract.submitTransaction('queryAlpha', 'Alice', 'Bob');

        // process response
        console.log('Process query response.');

        console.log(`Query of alpha value : successfully returned value ` + alphaResponse + `.`);
...
main().then(() => {
    ...
    console.log('Query alpha program complete.');

}).catch((e) => {

    console.log('Query alpha program exception.');
    console.log(e);
    console.log(e.stack);
    process.exit(-1);
